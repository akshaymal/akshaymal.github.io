...
4. **Self-verify** before review: run `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run test:viewport` locally. If `package.json` or `package-lock.json` changed, also run `npx --yes --package npm@10 -- npm ci --dry-run`. Fix any failures before proceeding.
