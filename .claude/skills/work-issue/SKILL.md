...
4. **Self-verify** before review: run `npm run lint`, `npm run typecheck`, and `npm run build` locally, plus `npx --yes --package npm@10 -- npm ci --dry-run` if `package.json` or `package-lock.json` changed. Fix any failures before proceeding.

   The `npm@10` dry-run exists because CI (`.github/workflows/ci.yml`) runs Node 20, which bundles a different npm major version than a typical local dev machine (this repo has been developed against npm 11.x locally) — `npm ci` can pass locally and still fail in CI purely from that version difference (see issue #12: two prior PRs shipped a lockfile that passed local `npm ci` but failed identically both times in CI, traced to an npm-version-dependent validation difference for an optional, cpu-filtered transitive dependency). Running the dry-run under npm 10 locally catches that class of drift before it reaches a PR. Skip it for changes that don't touch `package.json`/`package-lock.json` — it's not needed and adds ~10-20s for no benefit on unrelated changes.
