...
1. **Fetch and verify.** Read the issue using whichever tool is available in this session:
   ...
   - **Remote session** (`gh` not available): use the `mcp__github__issue_read` tool with `owner: akshaymal`, `repo: akshaymalhotra.dev`, `issue_number: <N>`
...
5. **Self-verify** before review. Run all of the following in order. Fix any failures before proceeding.
   ...
   If `package.json` or `package-lock.json` changed, also run `npx --yes --package npm@10 -- npm ci --dry-run` after the above.

   Capture each command's output to a log file rather than letting it flood the conversation — most runs pass, and there's no reason to carry hundreds of lines of routine build/test output through the rest of the session. On failure, grep the log for the tool's actual failure markers with a little context:
   ...
   **Notes on specific checks:**
   ...
   `node scripts/check-bundle-size.mjs` — budget check on the static export; requires `npm run build` to have produced `out/` first.

   `node scripts/check-internal-links.mjs` — scans the static export for broken internal links; also requires `out/`.

   `npm run test:viewport` — Playwright check (`e2e/viewport-overflow.spec.ts`) against the static export at four viewport widths per route, asserting nothing overflows horizontally. Added after a nav overflow bug shipped undetected because no tool in the harness rendered a real viewport (see issue #13). Also requires `out/`. **Chromium environment note:** in a remote cloud session (Claude Code on the web), Chromium is pre-installed at `PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers` — do not run `playwright install`. In a local session where the browser binary is missing or mismatched, run `npx playwright install chromium` once to fix it.

   `npm@10` dry-run: CI (`.github/workflows/ci.yml`) runs Node 20, which bundles a different npm major than a typical local machine (this repo has been developed against npm 11.x). Two prior PRs shipped a lockfile that passed local `npm ci` but failed CI identically both times (see issue #12). Running the dry-run under npm 10 locally is a cheap gate. Skip it for changes that don't touch `package.json`/`package-lock.json`.
6. **Independent review gate.** Dispatch a fresh subagent (not a fork — no shared context with this session) with the `code-review` skill. Give it a structured brief — not just the raw diff — so it spends tokens on the actual review rather than re-deriving context from scratch:
   ...
   - **Any known risk areas** — files or components that have caused bugs before, or that other parts of the codebase depend on heavily (e.g., shared layout components, the nav, data types in `content/`)
   - Instruct it to review for **correctness only**: logic bugs, missed edge cases, runtime misbehavior. Explicitly tell it to skip style, structure, and anything already enforced by lint or typecheck — those gates already ran.
...
9. **Open the PR.** Use whichever tool is available in this session:
   ...
   - **Remote session** (`gh` not available): use the `mcp__github__create_pull_request` tool with `owner: akshaymal`, `repo: akshaymalhotra.dev`, `head: <branch>`, `base: main`, `title`, and `body`
   ...
   - [x] npm run typecheck
   - [x] npm run build
   - [x] node scripts/check-bundle-size.mjs
   - [x] node scripts/check-internal-links.mjs
   - [x] npm run test:viewport
