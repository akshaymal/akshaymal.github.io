...
3. **Separate the stated request from the underlying goal.** If the issue already names a specific implementation ("add a sticky header," "lock the footer," "cache X in Redis") rather than describing an outcome, don't take that implementation at face value. Ask the user *why* — what breaks or is annoying without it — and confirm the named implementation is actually the best way to get there, not just the first mechanism that came to mind. Write the Acceptance Criteria around the *outcome* (what must be true for a visitor/user), not the literal mechanism, so a simpler or better implementation discovered during grilling — or during implementation itself — can still satisfy the issue. An issue that jumps straight to a specific fix for a vague problem is exactly the kind of issue that looks agent-ready but isn't: skip this step only when the issue is already phrased as an outcome with no implementation baked in.
