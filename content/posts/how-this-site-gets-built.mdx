...
```mermaid
flowchart LR
    A["Issue<br/>agent-ready"] --> B["work-issue<br/>implement + self-verify"]
    B --> C["Independent review<br/>fresh subagent"]
    C --> D["CI<br/>lint, build, viewport, Lighthouse"]
    D --> E["Human merge"]
```
...
```mermaid
pie title Verification touchpoints per change
    "Self-verify checks" : 6
    "CI jobs" : 3
