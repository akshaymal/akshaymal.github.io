...
- `render-mermaid.mjs` — runs automatically as the `prebuild` npm script, before `npm run build`; renders every ` ```mermaid ` fence in `content/posts/*.mdx` to static light/dark SVGs under `public/mermaid/` (gitignored, regenerated every build) via a headless Chromium.
...
- `workflows/ci.yml` — on every PR and push to `main`: `verify` job installs Chromium (for the Mermaid `prebuild` step) then runs lint, typecheck, build, the bundle-size check, and the internal-link check; a `viewport` job runs the Playwright viewport-overflow smoke test; a `lighthouse` job runs Lighthouse CI across every route (all jobs blocking).
